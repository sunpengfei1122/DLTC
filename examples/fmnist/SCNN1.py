...
class SNNLayer(object):
    def __init__(self, in_size, out_size, n_layer, name):
        ...
        self.weight = tf.get_variable(name, [self.in_size , self.out_size], initializer=tf.contrib.layers.xavier_initializer())
        self.in_size_real  = in_size
        self.delay = tf.get_variable(name+'delay', [self.in_size_real], initializer=tf.contrib.layers.xavier_initializer())
...
class SCNNLayer(object):
    def __init__(self, kernel_size=3, in_channel=1, out_channel=1, strides=1, n_layer_new=1, name='test'):
        self.MAX_SPIKE_TIME = 1e5
        ...
        self.kernel = SNNLayer(in_size=self.kernel_size * self.kernel_size * self.in_channel,
                                        out_size=self.out_channel, n_layer=n_layer_new, name=name)
